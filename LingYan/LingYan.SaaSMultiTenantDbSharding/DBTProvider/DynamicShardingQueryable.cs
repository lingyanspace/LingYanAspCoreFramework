...
{
    internal class DynamicShardingQueryable<T> : IDynamicShardingQueryable<T> where T : class 
    {
        ...
        public DynamicShardingQueryable(IQueryable<T> source, DynamicShardingDBTService shardingDb, IDynamicDBTConfig shardingConfig, IDynamicDBTFactory dbFactory)
        {
            _source = source; 
        ...
        DynamicShardingDBTService _shardingDb { get; }
        ...
        private async Task<List<TResult>> GetStatisDataAsync<TResult>(Func<IQueryable, Task<TResult>> access, IQueryable newSource = null)
        {
            ...
            SynchronizedCollection<IDynamicDBTService> dbs = new SynchronizedCollection<IDynamicDBTService>();
            tasks = tables.Select(aTable =>
            {
                IDynamicDBTService db;
                if (_shardingDb.OpenedTransaction)
                    db = _shardingDb.GetMapDbAccessor(aTable.conString, aTable.dbType, aTable.suffix);
                else
                    db = _dbFactory.GetDBTService(new DynamicDBCParamater
                    {
                        ConnectionString = aTable.conString,
                        DynamicDatabase = aTable.dbType,
        ...
        public IDynamicShardingQueryable<T> Where(Expression<Func<T, bool>> predicate)
        {
            _source = _source.Where(predicate);

            return this;
        }
        public IDynamicShardingQueryable<T> Where(string predicate, params object[] values)
        {
            _source = _source.Where(predicate, values);

            return this;
        }
        public IDynamicShardingQueryable<T> OrderBy<TKey>(Expression<Func<T, TKey>> keySelector)
        {
            _source = _source.OrderBy(keySelector);

            return this;
        }
        public IDynamicShardingQueryable<T> OrderByDescending<TKey>(Expression<Func<T, TKey>> keySelector)
        {
            _source = _source.OrderByDescending(keySelector);

            return this;
        }
        public IDynamicShardingQueryable<T> OrderBy(string ordering, params object[] values)
        {
            _source = _source.OrderBy(ordering, values);

            return this;
        }
        public IDynamicShardingQueryable<T> Skip(int count)
        {
            _source = _source.Skip(count);

            return this;
        }
        public IDynamicShardingQueryable<T> Take(int count)
        {
