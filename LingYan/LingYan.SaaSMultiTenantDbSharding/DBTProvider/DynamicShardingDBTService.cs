...
using LingYan.DynamicShardingDBT.DBTTransaction;
...
{
    internal class DynamicShardingDBTService : DefaultBaseDbService, IDynamicShardingDBTService
    {
        private readonly IDynamicDBTConfig _shardingConfig;
        private readonly IDynamicDBTFactory _dbFactory;

        #region 构造函数

        public DynamicShardingDBTService(IDynamicDBTConfig shardingConfig, IDynamicDBTFactory dbFactory)
        {
            _shardingConfig = shardingConfig;
            _dbFactory = dbFactory;
            var dbType = shardingConfig.FindADbType();
            _db = _dbFactory.GetDBTService(new DynamicDBCParamater { ConnectionString = null, DynamicDatabase = dbType });
        }
        ...
        private IDynamicDBTService _db { get; }
        private string GetDbId(string conString, DynamicDBType dbType, string suffix)
        {
        ...
        private async Task<int> PackAccessDataAsync(Func<Task<int>> access)
        {
            ...
            {
                _transaction.AddDBTService(dbs);
                count = await access();
            }

            return count;
        }
        private async Task<int> WriteTableAsync<T>(List<T> entities, Func<T, IDynamicDBTService, Task<int>> accessDataAsync)
        {
            List<(T obj, IDynamicDBTService db)> targetDbs = entities
                .Select(x => new
                {
                    Obj = x,
        ...
        private ConcurrentDictionary<string, IDynamicDBTService> _dbs { get; }
            = new ConcurrentDictionary<string, IDynamicDBTService>();
        ...
        public IDynamicDBTService GetMapDbAccessor(string conString, DynamicDBType dbType, string suffix)
        {
            var dbId = GetDbId(conString, dbType, suffix);
            IDynamicDBTService db = _dbs.GetOrAdd(dbId, key => _dbFactory.GetDBTService(new DynamicDBCParamater
            {
        ...
        public IDynamicShardingQueryable<T> GetIShardingQueryable<T>() where T : class
        {
            return new DynamicShardingQueryable<T>(_db.GetIQueryable<T>(), this, _shardingConfig, _dbFactory);
        }
